...
Inside the Railway-x world, managing your identity and data securely and efficiently is one of the key targets. Considering this, we implemented the usage of _Digital Wallet_ and _Credentials_ on Company level.
...
1. **Issuer**: Trusted entities in the dataspace issue digital credentials after verifying your identity or certifications. These credentials are then securely stored in your digital wallet.
   Trusted issuer of the Railway-x dataspace are certified and available via the 'SSI Authority & Schema Registry'.
