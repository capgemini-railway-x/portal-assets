...
Important: to enable the functionality, an idp connection must be existing between the business app and railway-x.  
This connection is a user federation which establishes trust between the app idp and railway-x idp and enabled users with a valid key to access there subscribed app tenant.
