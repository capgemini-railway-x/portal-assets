...
The Railway-X marketplace is hosted/operated by an CX operator. As a dataspace member (companies which have successfully registered for CX dataspace with one or several operators), app provider or service provider, roles can be selected.
